...
const hugGifs = require('./gifs/tickle-gifs.json');

module.exports = {
    data: new SlashCommandBuilder()
        .setName('tickle')
        .setDescription('Poke a tickle')
        .addUserOption(option =>
            option.setName('user')
                .setDescription('Select a user to tickle')
                .setRequired(true)),
    ...
    async execute(interaction) {
        ...
        if (!userToHug) {
            return interaction.reply('Please specify a user to tickle.');
        }
        ...
        const hugEmbed = new EmbedBuilder()
            .setDescription(`**${interaction.user}** tickles **${userToHug}**`)
            .setImage(randomHug);
