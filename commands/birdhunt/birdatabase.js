...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('birdatabase')
        .setDescription('Displays bird info: chances, names, emojis, and rarities, ordered from common/heavy to rare/light'),
    async execute(interaction) {
        try {
            ...
            const [birdsRaw, birdDataRaw] = await Promise.all([
                ...
                fs.readFile(path.join(__dirname, '../../birddata.json'), 'utf8')
            ]);
            ...
            const embed = new EmbedBuilder()
                .setColor('#1E90FF') // Set the embed color
                .setTitle('Birdatabase™')
                .setDescription('These are the bird types, their spawn Chances, and their Value.')

            // Add each bird to the embed
            joinedBirds.forEach(bird => {
                embed.addFields({
                    name: `${bird.emoji} **${bird.name}**`,
                    value: `Chance: ${bird.chance}%\nValue: ${bird.rarity}`,
                    inline: true,
                });
            });
