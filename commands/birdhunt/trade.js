...
// Load bird data from birddata.json
...
const birdData = loadJsonFile(path.join(__dirname, '../../birddata.json'));
const inventoriesPath = path.join(__dirname, '../../inventories.json');

function loadInventories() {
    try {
        const data = fs.readFileSync(inventoriesPath);
...
function saveInventories(inventories) {
    try {
        fs.writeFileSync(inventoriesPath, JSON.stringify(Object.fromEntries(inventories), null, 2));
    } catch (err) {
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const senderInventory = inventories.get(sender.id) || {};
        const targetInventory = inventories.get(target.id) || {};
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor('#1E90FF')
            .addFields(
                { name: `${sender.username} (Sender)`, value: sent.map(item => `${item.name} (x${item.quantity})`).join('\n') || 'No birds offered', inline: true },
                { name: `${target.username} (Recipient)`, value: received.map(item => `${item.name} (x${item.quantity})`).join('\n') || 'No birds offered', inline: true },
                { name: 'Total Value', value: `Sender: $${senderValue.toFixed(2)}\nRecipient: $${targetValue.toFixed(2)}\nImbalance: ${imbalanceString}`, inline: false }
            );
